...
public class DijkstraAlgorithm
{
    ...
    public static int Dijkstra(int[,] grid, int startX, int startY, int endX, int endY)
    {
        ...
        int[, , ,] distance = new int[rows, cols, 4, 4]; // 4 direction 3 max steps starting from 1
        bool[, , ,] visited = new bool[rows, cols, 4, 4]; // 4 direction 3 max steps starting from 1
        ...
        while (pq.Count > 0)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                int stepsSinceLastTurn = i == current.Direction ? current.StepsSinceLastTurn + 1 : 1;
